        ...
        <Card className="bg-slate-800/60 border-slate-600/50">
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2 items-center mb-4">
              ...
              {useDateFilter && (
                <>
                  <Input type="date" value={dateFrom} onChange={(e) => setDateFrom(e.target.value)} className="w-40 bg-slate-900 border-slate-600 text-white" placeholder="מתאריך" />
                  <Input type="date" value={dateTo} onChange={(e) => setDateTo(e.target.value)} className="w-40 bg-slate-900 border-slate-600 text-white" placeholder="עד תאריך" />
                </>
              )}
            </div>
            {isLoading ? (
