/**
 * Seed team library: Israeli Premier League (ליגת העל) teams.
 * Idempotent: skips teams that already exist (by categoryId + normalizedName).
 * Usage: pnpm run seed:team-library-ligat-haal
 */
...
const LIGAT_HAAL_SLUG = "ligat-haal";
...
async function main() {
  ...
  ).get(LIGAT_HAAL_SLUG) as { id: number } | undefined;
  if (!category) {
    console.error("Category ליגת העל (slug: ligat-haal) not found. Run: pnpm run seed:team-library");
    db.close();
    process.exit(1);
  }
  ...
  console.log("Ligat HaAl (ליגת העל) teams: inserted", inserted, ", skipped (already exist)", skipped);
  db.close();
}
