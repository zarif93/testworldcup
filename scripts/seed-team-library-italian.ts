/**
 * Seed team library: Italian league (איטלקית) teams.
 * Idempotent: skips teams that already exist (by categoryId + normalizedName).
 * Usage: pnpm run seed:team-library-italian
 */
...
async function main() {
  ...
  ).get(ITALIAN_SLUG) as { id: number } | undefined;
  if (!category) {
    console.error("Category איטלקית (slug: italian) not found. Run: pnpm run seed:team-library");
    db.close();
    process.exit(1);
  }
  ...
  console.log("Italian (איטלקית) teams: inserted", inserted, ", skipped (already exist)", skipped);
  db.close();
}
