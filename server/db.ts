...
async function getSubmissionFinancialRows(opts?: {
  ...
  if (opts?.tournamentType || opts?.sourceLabel) {
    const allowedIds = await getReportFilterTournamentIds({ tournamentType: opts.tournamentType, sourceLabel: opts.sourceLabel });
    if (allowedIds.length > 0) conditions.push(inArray(tournaments.id, allowedIds));
    else if (opts?.tournamentType) conditions.push(eq(tournaments.type, opts.tournamentType));
  }
...
export async function getPlayerPnL(
  userId: number,
  opts?: { from?: string; to?: string; tournamentType?: string; sourceLabel?: "legacy" | "universal"; status?: string }
): Promise<{
  profit: number;
  loss: number;
  net: number;
  ...
  const { getPlayerFinancialProfile } = await import("./finance");
  ...
  const useProfileTotals = profile && (profile.totalEntryFees > 0 || profile.totalPrizesWon > 0 || profile.totalEntryFeeRefunds > 0);
  const totalBets = useProfileTotals ? profile!.totalEntryFees - profile!.totalEntryFeeRefunds : entries.reduce((sum, entry) => sum + entry.betAmount, 0);
  const totalWinnings = useProfileTotals ? profile!.totalPrizesWon : entries.reduce((sum, entry) => sum + entry.winAmount, 0);
  const totalCommission = useProfileTotals ? profile!.totalCommissionGenerated : entries.reduce((sum, entry) => sum + entry.totalCommissionGenerated, 0);
  const totalProfitLoss = useProfileTotals ? profile!.competitionNetPnL : totalWinnings - totalBets;
  ...
  if (opts?.tournamentType || opts?.sourceLabel) {
    tournamentIds = await getReportFilterTournamentIds({ tournamentType: opts.tournamentType, sourceLabel: opts.sourceLabel });
    if (tournamentIds.length === 0) tournamentIds = null;
  }
...
export async function getAdminPnLReportRows(opts?: {
  ...
  const filterTournamentIds =
    opts?.tournamentType || opts?.sourceLabel
  ...
  for (const r of rows) {
    ...
    if (filterTournamentIds != null && (tournamentId == null || !filterTournamentIds.includes(tournamentId))) continue;
...
export async function getAgentPnLReportRows(
  agentId: number,
  opts?: { from?: string; to?: string; tournamentType?: string; sourceLabel?: "legacy" | "universal"; playerId?: number; limit?: number }
...
/** דוח רווח והפסד לסוכן – canonical source: getAgentDashboardMetrics (financial_events). Fallback to legacy when no events data. */
export async function getAgentPnL(
  ...
  const canonicalTotals =
    metrics &&
    (metrics.playerListWithPnL.some((p) => p.totalEntryFees > 0 || p.totalPrizesWon > 0) || metrics.agentTotalCommissionEarned > 0);
  if (!canonicalTotals) {
    ...
    return {
      profit: Math.max(totalProfitLoss, 0),
      loss: Math.max(-totalProfitLoss, 0),
      net: totalProfitLoss,
      totalBets,
      totalWinnings,
      totalProfit: Math.max(totalProfitLoss, 0),
      totalLoss: Math.max(-totalProfitLoss, 0),
  ...
  const totalBets = metrics!.playerListWithPnL.reduce((s, p) => s + (p.totalEntryFees - p.totalEntryFeeRefunds), 0);
  const totalWinnings = metrics!.playerListWithPnL.reduce((s, p) => s + p.totalPrizesWon, 0);
  const totalProfitLoss = totalWinnings - totalBets;
  const totalProfit = Math.max(totalProfitLoss, 0);
  const totalLoss = Math.max(-totalProfitLoss, 0);
  const totalPlatformCommission = metrics!.totalCommissionGenerated - metrics!.agentTotalCommissionEarned;
  const totalAgentCommission = metrics!.agentTotalCommissionEarned;

  const players = metrics!.playerListWithPnL
    .map((p) => ({
      playerId: p.userId,
      username: p.username,
      name: null as string | null,
      phoneNumber: null as string | null,
      totalBets: p.totalEntryFees - p.totalEntryFeeRefunds,
      totalWinnings: p.totalPrizesWon,
      profitLoss: p.competitionNetPnL,
      commissionGenerated: p.commissionGenerated,
      agentCommissionShare: p.agentCommissionFromPlayer,
      betsCount: 0,
      lastActivity: null as Date | null,
    }))
...
/** דוח רווח והפסד לכל שחקן של סוכן – לטבלה בדף סוכן. אופציונלי: סינון לפי סוג תחרות ומקור פריטים. */
...
/** סיכום דוח רווח והפסד למנהל – כל השחקנים וכל הסוכנים בטווח. אופציונלי: סינון לפי סוג תחרות ומקור פריטים. */
export async function getAdminPnLSummary(opts?: { from?: string; to?: string; tournamentType?: string; sourceLabel?: "legacy" | "universal" }): Promise<{
  ...
  const tournamentIds = (opts?.tournamentType || opts?.sourceLabel)
    ? await getReportFilterTournamentIds({ tournamentType: opts.tournamentType, sourceLabel: opts.sourceLabel })
    : [];
  ...
  const commissionRows =
    tournamentIds.length > 0
      ? await db
          ...
          .innerJoin(tournaments, eq(submissions.tournamentId, tournaments.id))
          .where(and(...acConditions, inArray(tournaments.id, tournamentIds)))
